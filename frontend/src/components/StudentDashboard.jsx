...
function StudentDashboard() {
    ...
    useEffect(() => {
        let timer;
        if (showCertificateModal && certModalTimer > 0) {
            timer = setTimeout(() => {
                setCertModalTimer(prev => prev - 1);
            }, 1000);
        }
        return () => clearTimeout(timer);
    }, [showCertificateModal, certModalTimer]);
    ...
    const proceedWithSubmission = async (targetSession, targetAssignment) => {
        const key = `${targetSession._id}-${targetAssignment.title}`;
        const data = assignmentData[key];

        try {
            const formData = new FormData();
            formData.append('sessionId', targetSession._id);
            formData.append('assignmentTitle', targetAssignment.title);
            formData.append('assignmentType', targetAssignment.type);

            if (targetAssignment.type === 'file' && data instanceof File) {
            ...
            if (targetAssignment.title === 'Certificate' || targetAssignment.type === 'certificate') {
                setRedoActive({ ...redoActive, [targetSession._id]: false });
                setShowCertificateModal(false);
                setPendingCertData(null);
            }
    ...
    const handleAssignmentSubmit = async (session, assignment) => {
        ...
        if (assignment.type === 'certificate' || assignment.title === 'Certificate') {
            setPendingCertData({ session, assignment });
            setShowCertificateModal(true);
            setCertModalTimer(5);
            return;
        }

        proceedWithSubmission(session, assignment);
    };
                                                                            ...
                                                                            <div className="bg-white/40 p-4 rounded-xl border border-white/60 backdrop-blur-sm shadow-sm animate-in fade-in slide-in-from-top-4 duration-300">
                                                                                ...
                                                                                <div className="mt-4 p-4 bg-red-50 border border-red-100 rounded-xl space-y-3">
                                                                                    <div className="flex items-start gap-3">
                                                                                        ...
                                                                                        <div className="space-y-1">
                                                                                            <p className="text-[10px] font-black text-red-800 uppercase tracking-widest">Mandatory Important Notice</p>
                                                                                            <p className="text-[10px] text-red-700 font-bold leading-relaxed">
                                                                                                "Certificate credentials will be checked individually. Do not submit fake or edited certificates. It leads to serious consequences."
                                                                                            </p>
                                                                                        </div>
                                                                                    </div>
                ...
                {showCertificateModal && (
                    <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
                        <motion.div
                            ...
                            className="bg-white w-full max-w-md rounded-[2.5rem] overflow-hidden shadow-2xl relative border border-white/20"
                        >
                            <div className="bg-gradient-to-br from-red-600 to-red-800 p-8 text-white relative overflow-hidden">
                                ...
                                <div className="relative z-10 flex flex-col items-center text-center space-y-4">
                                    <div className="p-4 bg-white/20 rounded-2xl backdrop-blur-md ring-1 ring-white/30 shadow-inner">
                                        <AlertCircle className="w-10 h-10 text-white" />
                                    </div>
                                    <div>
                                        <h3 className="font-black text-2xl uppercase tracking-tighter leading-none">Final Warning</h3>
                                        <p className="text-red-100/80 text-[10px] font-black uppercase tracking-[0.3em] mt-2">Integrity Verification</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-8 sm:p-10 space-y-8">
                                <div className="space-y-6">
                                    <div className="space-y-4">
                                        <div className="p-6 rounded-[2rem] bg-red-50/50 border border-red-100 shadow-inner relative group">
                                            <p className="text-zinc-900 font-bold text-lg sm:text-xl leading-relaxed text-center italic">
                                                "Certificate credentials will be checked individually. Do not submit fake or edited certificates. It leads to serious consequences."
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-3">
                                        <div className="flex items-center gap-3 px-4 py-2 bg-zinc-50 rounded-xl border border-zinc-100">
                                            ...
                                            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Manual Verification Protocol Active</p>
                                        </div>
                                        <div className="flex items-center gap-3 px-4 py-2 bg-zinc-50 rounded-xl border border-zinc-100">
                                            <div className="w-1.5 h-1.5 rounded-full bg-zinc-300" />
                                            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Digital Footprint Logged</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col gap-3">
                                    <button
                                        onClick={() => proceedWithSubmission(pendingCertData.session, pendingCertData.assignment)}
                                        disabled={certModalTimer > 0}
                                        className="w-full py-5 rounded-2xl bg-zinc-900 hover:bg-black text-white font-black uppercase tracking-[0.2em] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 shadow-2xl shadow-zinc-900/40 active:scale-[0.98]"
                                    >
                                        {certModalTimer > 0 ? (
                                            <>
                                                <Clock className="w-5 h-5 animate-spin" />
                                                Confirm in {certModalTimer}s
                                            </>
                                        ) : (
                                            <>
                                                <CheckCircle className="w-5 h-5" />
                                    ...
                                    <button
                                        ...
                                        className="w-full py-4 text-zinc-400 hover:text-zinc-900 text-[10px] font-black uppercase tracking-[0.3em] transition-colors"
                                    >
                                        Cancel Submission
                                    </button>
                                </div>
